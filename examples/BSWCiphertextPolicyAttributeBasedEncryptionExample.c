...
int main()
{
    ...
    */
    ...
    bswCiphertextPolicyAttributeBasedEncryptionEncryptedMessageAsBinary* ciphertext = malloc(sizeof (bswCiphertextPolicyAttributeBasedEncryptionEncryptedMessageAsBinary));
    if (CRYPTID_SUCCESS != cryptid_abe_bsw_encrypt(ciphertext, accessTreeAsBinary, message, strlen(message), publicKey))
    {
        printf("Encrypt failed\n");
        return -1;
    }

    bswCiphertextPolicyAttributeBasedEncryptionSecretKeyAsBinary* privateKey  = malloc(sizeof (bswCiphertextPolicyAttributeBasedEncryptionSecretKeyAsBinary));
    if (CRYPTID_SUCCESS != cryptid_abe_bsw_keygen(privateKey, masterKey, attributes, numOfAttributes))
    {
    ...
    char *plaintext;
    if (CRYPTID_SUCCESS != cryptid_abe_bsw_decrypt(&plaintext, ciphertext, privateKey))
    {
    ...
    free(plaintext);
    bswCiphertextPolicyAttributeBasedEncryptionEncryptedMessageAsBinary_destroy(ciphertext);
    bswCiphertextPolicyAttributeBasedEncryptionSecretKeyAsBinary_destroy(privateKey);
