 ...
 * @param result Out parameter storing an integer in the range \f$0\f$ to \f$p -
 * 1\f$. Must be mpz_init'd and mpz_clear'd by the caller.
 ...
 * @param result Out parameter storing a point of order \f$q\f$ in \f$E(F_p)\f$.
 * On CRYPTID_SUCCESS, it must be destroyed by the caller.
 * @param id a string
 ...
 * @param result out parameter storing the resulting string of size \f$2 \cdot
 ...
 * @param b The length of the result. Must be less than or equal to the number
 * of bytes in the output of the hash function.
