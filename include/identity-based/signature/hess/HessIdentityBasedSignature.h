 ...
 * @param publicParametersAsBinary Pointer in which the public parameters will
 * be stored. If the return value is CRYPTID_SUCCESS then it will point to
 * correctly filled HessIdentityBasedSignaturePublicParameters instance
 ...
 * @param result Out parameter holding the private key (which is an
 * AffinePoint). On CRYPTID_SUCCESS, it must be destroyed by the caller.
 ...
 * @param masterSecretAsBinary the master secret corresponding to the public
 * parameters
 ...
 * @param result Out parameter storing the signature. If the return value is
 * CRYPTID_SUCCESS then it will point to a
 * HessIdentityBasedSignatureSignatureAsBinary instance, that must be destroyed
 * by the caller. Initialization is done by this function.
 ...
 * @param messageLength the length of the message
 * @param signatureAsBinary the digital signature of the message, which needs to
 * be verified
