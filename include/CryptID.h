 ...
 * @param publicParameters Pointer in which the public parameters will be stored. If the return value is SUCCESS
 ...
 * @param result Out parameter holding the private key (which is an AffinePoint). On SUCCESS, it must be destroyed by 
 ...
 * @param result Out parameter storing the ciphertext. If the return value is SUCCESS
 ...
 * @param result Out parameter holding the message in plaintext. If the return value is SUCCESS
 *               then it will point to a zero-terminated string, that must be destroyed by the caller. 
 * @param privateKey the private key to decrypt with
 * @param ciphertext the ciphertext to decrypt
 * @param publicParameters the IBE public parameters
 * @return SUCCESS of everything went right
 */
