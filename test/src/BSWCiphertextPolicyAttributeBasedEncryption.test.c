...
TEST basic_abe_test(
    SecurityLevel securityLevel, char *message,
    bswCiphertextPolicyAttributeBasedEncryptionAccessTreeAsBinary
        *accessTreeAsBinary,
    char **attributes, int numAttributes, int expectedReponse) {
...
static void generateRandomString(char **output, size_t outputLength,
                                 char *alphabet, size_t alphabetSize) {
...
SUITE(cryptid_abe_suite) {
  ...
  for (int i = 0; i < 2; i++) {
    ...
    bswCiphertextPolicyAttributeBasedEncryptionAccessTreeAsBinary
        *accessTreeAsBinary =
            bswCiphertextPolicyAttributeBasedEncryptionAccessTreeAsBinary_init(
                (i % 2) * (numChilds - 1) + 1, NULL, 0, numChilds);
    ...
    for (int a = 0; a < numChilds; a++) {
      generateRandomString(&randomStr, 20, defaultAlphabet,
                           strlen(defaultAlphabet));
      if (expectedReponse == 0) {
        generateRandomString(&randomStr2, 20, defaultAlphabet,
                             strlen(defaultAlphabet));
      } else {
      ...
      bswCiphertextPolicyAttributeBasedEncryptionAccessTreeAsBinary *child =
          bswCiphertextPolicyAttributeBasedEncryptionAccessTreeAsBinary_init(
              1, randomStr2, strlen(randomStr2), 0);
      accessTreeAsBinary->children[a] = child;
      attributes[a] = malloc(strlen(randomStr) + 1);
      strcpy(attributes[a], randomStr);
    }

    RUN_TESTp(basic_abe_test, LOWEST, message, accessTreeAsBinary, attributes,
              numAttributes, expectedReponse);
  ...
  RUN_TESTp(basic_abe_test, LOWEST, message, accessTreeAsBinary, attributesGood,
            numAttributes, 1);
  RUN_TESTp(basic_abe_test, LOWEST, message, accessTreeAsBinary, attributesBad,
            numAttributes, 0);
