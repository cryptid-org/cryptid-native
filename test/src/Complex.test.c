...
TEST additiveInverse_should_return_the_additive_inverse(const Complex complex,
...
SUITE(additive_inverse_suite) {
...

  {
    ...
    RUN_TESTp(additiveInverse_should_return_the_additive_inverse, c, expected,
              p);
...
TEST the_modulo_power_of_complex_numbers_should_work_well(
    const Complex base, const mpz_t exp, const mpz_t p,
    const Complex expected) {
  // When
  Complex result;
  complex_modPow(&result, base, exp, p);
...
SUITE(modulo_power_suite) {
...

  {
    ...
    RUN_TESTp(the_modulo_power_of_complex_numbers_should_work_well, base, exp,
              p, expected);
...
TEST GF_5_modMul_should_just_work(const long scalar, const long expectedReal,
                                  const long expectedImaginary) {
  // Given
  mpz_t p, s;
  mpz_init_set_ui(p, 5);
...
TEST GF_7_multiplying_an_element_with_its_multiplicative_inverse_should_yield_the_identity_element(
    const long real, const long imaginary) {
  // Given
  Complex id;
  complex_initLong(&id, 1, 0);
  mpz_t p;
...
SUITE(multiplicative_inverse_suite) {
...

  {
    ...
    for (long n = 1; n < (p * p - 1); ++n) {
      ...
      RUN_TESTp(
          GF_7_multiplying_an_element_with_its_multiplicative_inverse_should_yield_the_identity_element,
          real, imaginary);
    }
  }
}
